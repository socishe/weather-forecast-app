...
const WeatherCard = () => {
  ...
  useEffect(() => {
    ...
    const URL = `https://socishe.github.io/react-weather-app/http://api.openweathermap.org/data/2.5/weather?q=${state.city}&units=metric&appid=${API_KEY}`;
  ...
  const makeCall = new Promise((resolve, reject) => {
    const URL = `https://socishe.github.io/react-weather-app/http://api.openweathermap.org/data/2.5/forecast?q=${state.city}&units=metric&appid=${API_KEY}`;
